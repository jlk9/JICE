...
function run_ice_adjoint_hT_step(jcmodel, atmodel, step, ∂f_∂h, ∂f_∂T_new)
    ...
    autodiff(run_ice_step, Const, Const(jcmodel.N_i), Const(jcmodel.N_t),
            ...
            Const(atmodel.F_sw), Const(atmodel.F_Ld), Const(atmodel.T_a),
...
function run_ice_adjoint_Tw_step(jcmodel, step, ∂f_∂h, ∂f_∂T_new)
    ...
    Duplicated(jcmodel.z_new, ∂f_∂z_new), Const(jcmodel.F_0[step]), Const(jcmodel.dF_0[step]), Duplicated(jcmodel.maindiag, ∂f_∂maindiag),
